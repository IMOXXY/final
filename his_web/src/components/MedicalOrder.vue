...
<div class="order-box">
    ...
    <div class="order-header">
        ...
        <div> 
            ...
            <el-button class="order-pay" type="info" @click="toOrderPay">支付</el-button>
    ...
    <div class="data-table-show">
        <el-table
            ...
            border
            height="380"
            :row-class-name="orderTypeRowClass"
        >
            ...
            >
                <template slot-scope="scope">
                    <span v-if="k === 'ispaid'">{{ scope.row.ispaid ? '是' : '否' }}</span>
...
<el-dialog :visible.sync="orderToPayCtr.visible" width="70%">
    ...
    <div class="order-pay-table-container">
        ...
        <div class="qrcode-container">
            <qrcode-vue :value="orderToPayCtr.payUrl" :key="orderToPayCtr.payUrl" :size="160" />
...
export default {
    ...
    data() {
        return {
            ...
            orderShowColumns: {
                ...
                ispaid: { desc: '是否付费', width: 100, formatter: true },
    ...
    methods: {
        ...
        async getDoctorOrders() {
            ...
            if (!patiInfo) return
            const {data:res} = await this.$http.get('/doctororder/rid/'+patiInfo.rid)
            if (res.code===200) {
                this.doctorOrders = res.data
            }
        },
        async toOrderPay(){
            ...
            if (patiInfo) {
                this.orderToPayCtr.payUrl = "https://diabetes-polar-coffee.ngrok-free.dev/#/outbillpay?rid="+patiInfo.rid+"&selectedOrders=&token="+sessionStorage.getItem("token")
        ...
        onPayOrderSelect(orderRows) {
            ...
            const unpaidOrders = this.orderToPayCtr.orderToPayDataTable.filter(o => this.orderToPayCtr.selectedOrder.includes(o.orderid))
        ...
        // 根据医嘱类型设置行背景色
        orderTypeRowClass({ row }) {
            if (row.ordertype === '药品') return 'row-drug'
            if (row.ordertype === '检查') return 'row-check'
            if (row.ordertype === '治疗') return 'row-treat'
            if (row.ordertype === '检验') return 'row-lab'
            return ''
        },
